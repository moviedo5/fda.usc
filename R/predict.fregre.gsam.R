...
#fnf2<- NULL
fnf2 <- object$fnf2
specials1<-specials2<-fnf1<-fnf<-bs.dim1<-bs.dim2<-vfunc2<-vfunc<-vnf<-NULL
nterms<-length(terms)
...
k=1
 ...
 for (i in 1:lenfunc) {
   if(class(newx[[vfunc[i]]])[1]=="fdata"){
      ...
      if (is.null(basis.b[[vfunc[i]]])& bsp1)  basis.b[[vfunc[i]]]<-basis.x[[vfunc[i]]]#create.fdata.basis(fdat)
      ...
      if (bsp1 & bsp2) {
          ...
          #xcc<-fdata.cen(data[[vfunc[i]]],func.mean(object$data[[vfunc[i]]]))
          xcc<-list(data[[vfunc[i]]],func.mean(object$data[[vfunc[i]]]))
      ...
      x.fd = Data2fd(argvals = tt, y = t(xcc[[1]]$data)
       ...
       #  Z =t(x.fd$coefs) %*% J
         ...
         colnames(J)=colnames(Z) = name.coef[[vfunc[i]]]=paste(vfunc[i],".",basis.b[[vfunc[i]]]$names,sep="")
            ...
            Z =t(x.fd$coefs) 
            colnames(Z) = name.coef[[vfunc[i]]]=paste(vfunc[i],".",basis.b[[vfunc[i]]]$names,sep="")
            }
...
#             print( bs.dim2[i])
        ...
        xcen<-fdata.cen(fdat,func.mean(object$data[[vfunc[i]]]))[[1]]
        ...
        Z<- inprod.fdata(xcen,object$basis.list[[vfunc[i]]])
        response = "y"
...
# ##############################<
# res=fregre.gsam(Fat~x+s(x.d1),ldata,family=gaussian())
# res=fregre.gsam(Fat~s(Protein)+s(x),data=ldata,family=gaussian())
# summary(res)
# class(ldata)<-c("ldata","list")
# pred <-predict.fregre.gsam(res,ldata[1:11,row=T])
# plot(ldata$df$Fat[1:11],res$fitted.values[1:11])
# points(ldata$df$Fat[1:11],pred,col=2)
# 
# pred2meas(ldata$df$Fat,res$fitted.values)
# pred2meas(ldata$df$Fat,pred)
# 
# 
# 
# res=fregre.gsam(Fat~x+s(x.d1),ldata,family=gaussian())
